...
final class FocusShortcutTests: XCTestCase {
    ...
    func testOnHoldsUntilTurnedOffAndNamesNoFocus() throws {
        let params = try parameters(
            FocusShortcut.plistData(recipe: recipe, direction: .turnOn)
        )
        XCTAssertEqual(params["Enabled"] as? Int, 1)
        XCTAssertEqual(params["AssertionType"] as? String, "Turned Off")
        // No Focus is named on purpose: reading the user's Focus modes needs Full Disk Access, so
        // the action ships unset and Shortcuts renders it as Do Not Disturb, retargetable in place.
        XCTAssertNil(params["FocusModes"])
    }

    func testOffOmitsTheAssertionAndNamesNoFocus() throws {
        let params = try parameters(
            FocusShortcut.plistData(recipe: recipe, direction: .turnOff)
        )
    ...
    func testOutputIsABinaryPlist() throws {
        let data = try FocusShortcut.plistData(recipe: recipe, direction: .turnOn)
